...
class MovieCollectionSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        collection_movies = [
            MovieGenre(collection=collection, movie_uuid=movie['uuid'], title=movie['title'], description=movie['description'], genres=movie['genres']) for movie in validated_data['movies']
        ]
    ...
    def update(self, instance, validated_data):
        if len(validated_data['movies']) > 0:
            ...
            collection_movies = [
                MovieGenre(collection=instance, movie_uuid=movie['uuid'], title=movie['title'], description=movie['description'], genres=movie['genres']) for movie in validated_data['movies']
            ]
